...
def print_return_metrics(y_true, y_pred, label):
    ...
    for idx, name in enumerate(HORIZONS):
        ...
        print(
            f"{name:>2}: MSE={mse:.2f}, RMSE={rmse:.2f}, MAE={mae:.2f}, SignAcc={sign_acc:.2f}%"
        )
...
def print_price_metrics(y_true, y_pred, label):
    ...
    for idx, name in enumerate(HORIZONS):
        ...
        r2 = r2_score(t, p)
        # Directional accuracy: did the model predict the right price direction?
        dir_acc = np.mean(np.sign(p - t[:1]) == np.sign(t - t[:1])) * 100
        print(
            f"{name:>2}: MSE={mse:.2f}, RMSE={rmse:.2f}, MAE={mae:.2f}, MAPE={mape:.2f}%, R²={r2:.4f}, DirAcc={dir_acc:.1f}%"
        )
